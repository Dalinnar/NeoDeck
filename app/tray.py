...
def reload_config():
    ...
    return (
        cfg.port,
        cfg.language,
        cfg.open_settings_in_integrated_browser
    )
...
def toggle_console():
    ...
    try:
        loaded_settings["neodeck"]["show_console"] = (
            not loaded_settings["neodeck"]["show_console"]
        )
...
def create_language_items():
    ...
    def mk_item(lang):
        ...
        return pystray.MenuItem(
            ...
            radio=True,
            checked=lambda item: (
                lang['code'] == get_language(language)
            )
        )
...
def generate_tray_icon():
    ...
    if icon is None:
        ...
        icon = pystray.Icon(
            "Neodeck",
            Image.open("static/icons/icon.ico"),
            "Neodeck",
            generate_menu(SERVER_STATE)
        )
    ...
    return pystray.Menu(
        ...
        pystray.MenuItem(
            ...
            pystray.Menu(
                ...
                pystray.MenuItem(
                    text('open_plugins_folder'),
                    open_plugins_folder
                ),

                pystray.MenuItem(
                    text('language'),
                    pystray.Menu(*create_language_items())
                ),

                pystray.MenuItem(
                    text('restart_application'),
                    restart_program
                ),

                pystray.MenuItem(
                    text('edit_port'),
                    lambda: change_port_prompt()
                ),
        ...
        pystray.MenuItem(
            f"{text('server_status')} {status_text.get(server_status)}",
            open_config
        ),
        ...
        pystray.MenuItem(
            ...
            lambda: webbrowser.open(
                'https://github.com/Dalinnar/Neodeck/issues'
            )
        ),
